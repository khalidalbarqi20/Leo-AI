...
SYSTEM_PROMPT = "You are a fast coding assistant. Be concise. Write code in Markdown blocks. For multiple files use ### filename.ext. Always respond in Arabic."
...
async def chat(request: Request, prompt: str = Form(...), file: UploadFile = File(None)):
    ...
    if file and file.filename:
        try:
            ...
            if mt.startswith("image/"):
                ...
                content.append({"type": "image_url", "image_url": {"url": "data:" + mt + ";base64," + b64}})
            else:
                try:
                    td = fc.decode("utf-8")
                    content[0]["text"] = content[0]["text"] + chr(10) + chr(10) + "[File " + file.filename + "]:" + chr(10) + td
                except:
                    content[0]["text"] = content[0]["text"] + chr(10) + chr(10) + "(Non-text: " + file.filename + ")"
        except Exception as e:
            logger.error("File: " + str(e))
    ...
    for model in FALLBACK_MODELS:
        try:
            r = requests.post(
                ...
                headers=headers,
                json={"model": model, "messages": msgs, "temperature": 0.7, "max_tokens": 2000},
                timeout=8
            )
            j = r.json()
    ...
    if not ai_resp:
        ai_resp = "All models failed. Error: " + last_err
