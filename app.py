...
chat_sessions: dict = {}
stop_flags:    dict = {}          # session_id → asyncio.Event
...
SYSTEM_PROMPT = (
    ...
    "اكتب الكود دائماً داخل بلوكات Markdown. إذا كانت هناك عدة ملفات استخدم ### filename.ext. "
...
def read_file_content(filename: str, raw_bytes: bytes, content_type: str) -> str:
    ...
    try:
        ...
        return f"(لم يمكن قراءة: {filename})"
...
async def stop_stream():
    """يُوقف البث الجاري للجلسة الافتراضية."""
    sid = "default"
...
async def chat_stream(
    ...
    ordered = VISION_IDS if has_image else ([chosen] + [mid for mid in valid_ids if mid != chosen])
    ...
    # ── إنشاء stop flag جديد لهذا الطلب ─────────────────────────────────────
    stop_ev = asyncio.Event()
    stop_flags[sid] = stop_ev

    msg_content = [{"type":"text","text":user_text}, img_part] if img_part else user_text

    hdr  = {"Authorization": f"Bearer {OPENROUTER_API_KEY}", "Content-Type": "application/json"}
    msgs = [{"role":"system","content":SYSTEM_PROMPT}]
    msgs.extend(session[-8:])
    msgs.append({"role":"user","content":msg_content})

    # ── دالة streaming الحقيقية ────────────────────────────────────────────────
    async def streamer():
        full_text  = ""
        ...
        for mid in ordered:
            ...
            try:
                    ...
                    async with client.stream(
                        ...
                        if resp.status_code != 200:
                            ...
                            try:
                                ...
                                last_err = err_j.get("error",{}).get("message", str(resp.status_code))
                            except Exception:
                                last_err = str(resp.status_code)
                            logger.warning(f"Model {mid} → HTTP {resp.status_code}: {last_err}")
                            continue
                ...
                last_err = "timeout"
                logger.warning(f"Model {mid} timed out")
                continue
            except Exception as exc:
                last_err = str(exc)
                logger.warning(f"Model {mid} error: {exc}")
                continue

        # ── إذا أُوقف بواسطة المستخدم ─────────────────────────────────────────
