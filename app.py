...
app = FastAPI(title="Leo-AI", version="7.0.0")
...
def check_admin(token: str | None):
    """Return 403 JSON if not admin."""
    if not is_admin(token):
        return JSONResponse({"error": "هذه الميزة للمستخدم الرئيسي فقط"}, status_code=403)
    return None

def gh_headers() -> dict:
    h = {"Accept": "application/vnd.github.v3+json", "User-Agent": "Leo-AI/7.0"}
    if GITHUB_TOKEN:
...
def build_project_context(sid):
    ...
    for fname,content in mem.items(): parts.append(f"\n### {fname}\n{content[:3000]}")
...
async def github_load(owner:str=Form(...), repo:str=Form(...), branch:str=Form("main"),
    ...
    project_memory.setdefault(sid,{}); added=[]; errors=[]
    async with httpx.AsyncClient(timeout=20, headers=gh_headers()) as client:
...
8. **💡 اقتراحات التحسين**"""
    ...
    messages=[{"role":"system","content":SYSTEM_PROMPT_ADMIN},{"role":"user","content":review_prompt}]
...
async def chat_stream(request:Request, prompt:str=Form(...), model_id:str=Form(None),
    ...
    role=get_role(leo_session)
    # Guests get separate session namespace
